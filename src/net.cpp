...
void CNode::copyStats(CNodeStats& stats)
{
    ...
    // Raw ping time is in microseconds, but show it to user as whole seconds (TUNEZ users should be well used to small numbers with many decimal places by now :)
...
void ThreadMapPort()
{
    ...
    if (r == 1) {
        ...
        string strDesc = "TUNEZ " + FormatFullVersion();
...
bool BindListenPort(const CService& addrBind, string& strError, bool fWhitelisted)
{
    ...
    if (::bind(hListenSocket, (struct sockaddr*)&sockaddr, len) == SOCKET_ERROR) {
        ...
        if (nErr == WSAEADDRINUSE)
            strError = strprintf(_("Unable to bind to %s on this computer. TUNEZ Core is probably already running."), addrBind.ToString());
        else
...
static bool IsUpdateAvailable(CUrl& redirect)
{
    ...
    if (!CURLGetRedirect(urlRelease, redirect, error)) {
        ...
        if (urlRelease == GITHUB_RELEASE_URL) {
            boost::algorithm::replace_all(urlRelease, "TunezCoinNG", "TunezNG");
            if (!CURLGetRedirect(urlRelease, redirect, error)) {
                DebugPrintf("%s: error - %s\n", __func__, error);
                return false;
            }
