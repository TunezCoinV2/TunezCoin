...
{
    return QCoreApplication::translate("tunez-core", psz).toStdString();
}
...
static void initTranslations(QTranslator& qtTranslatorBase, QTranslator& qtTranslator, QTranslator& translatorBase, QTranslator& translator)
{
    ...
    // Load e.g. bitcoin_de.qm (shortcut "de" needs to be defined in tunez.qrc)
    if (translatorBase.load(lang, ":/translations/"))
        QApplication::installTranslator(&translatorBase);

    // Load e.g. bitcoin_de_DE.qm (shortcut "de_DE" needs to be defined in tunez.qrc)
...
/** Class encapsulating TunezNG Core startup and shutdown.
...
/** Main TunezNG application object */
...
void BitcoinApplication::initializeResult(int retval)
{
    ...
    if (retval) {
        ...
        // TunezNG: URIs or payment requests:
...
{
    QMessageBox::critical(0, "Runaway exception", BitcoinGUI::tr("A fatal error occurred. TunezNG can no longer continue safely and will quit.") + QString("\n\n") + message);
    ::exit(1);
}
...
/// 2. Basic Qt initialization (not dependent on parameters or configuration)
    ...
    Q_INIT_RESOURCE(tunez_locale);
    Q_INIT_RESOURCE(tunez);
    ...
    try {
        ...
        oldSettingsPath.replace(QString(QAPP_ORG_NAME), QString("TunezCoinNG"));
    ...
    /// 6. Determine availability of data directory and parse tunez.conf
    /// - Do not call GetDataDir(true) before this step finishes
    if (!boost::filesystem::is_directory(GetDataDir(false))) {
        QMessageBox::critical(0, QObject::tr("TunezNG Core"),
    ...
    if (!SelectParamsFromCommandLine()) {
        QMessageBox::critical(0, QObject::tr("TunezNG Core"), QObject::tr("Error: Invalid combination of -regtest and -testnet."));
        return 1;
    }
    ...
    if (bootstrap) {
        try {
            ...
            QMessageBox::critical(0, QObject::tr("TunezNG Core"), QObject::tr("Bootstrap failed, unexpected error. Please restart wallet."));
            return 1;
        }
    }
    ...
    // tunez: links repeatedly have their payment requests routed to this process:
    ...
    try {
        ...
        WinShutdownMonitor::registerShutdownBlockReason(QObject::tr("TunezNG Core didn't yet exit safely..."), (HWND)app.getMainWinId());
#endif
        app.exec();
        app.requestShutdown();
        app.exec();
