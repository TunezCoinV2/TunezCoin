                                                          ...
                                                          currentBalance(-1)
{
    ...
    // "Spending 999999 zTUNEZ ought to be enough for anybody." - Bill Gates, 2017
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_20_ZEROCOIN_MAINTENANCE_MODE)) {
        ...
        ui->pushButtonMintzPIV->setToolTip(tr("zTUNEZ is currently disabled due to maintenance."));

        ui->pushButtonSpendzPIV->setEnabled(false);
        ui->pushButtonSpendzPIV->setToolTip(tr("zTUNEZ is currently disabled due to maintenance."));
    }
}
...
void PrivacyDialog::on_pushButtonMintzPIV_clicked()
{
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_20_ZEROCOIN_MAINTENANCE_MODE)) {
        QMessageBox::information(this, tr("Mint Zerocoin"),
                                 tr("zTUNEZ is currently undergoing maintenance."), QMessageBox::Ok,
                                 QMessageBox::Ok);
        return;
    }
    ...
    ui->TEMintStatus->setPlainText(tr("Minting ") + ui->labelMintAmountValue->text() + " zTUNEZ...");
    ...
    QString strStatsHeader = tr("Successfully minted ") + ui->labelMintAmountValue->text() + tr(" zTUNEZ in ") +
...
void PrivacyDialog::on_pushButtonSpendzPIV_clicked()
{
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_20_ZEROCOIN_MAINTENANCE_MODE)) {
        QMessageBox::information(this, tr("Mint Zerocoin"),
                                 tr("zTUNEZ is currently undergoing maintenance."), QMessageBox::Ok, QMessageBox::Ok);
        return;
    }
    ...
    if (encStatus == walletModel->Locked || encStatus == walletModel->UnlockedForAnonymizationOnly) {
        ...
        // Wallet is unlocked now, send zTUNEZ
        sendzPIV();
        return;
    }
    // Wallet already unlocked or not encrypted at all, send zTUNEZ
    sendzPIV();
}
...
void PrivacyDialog::sendzPIV()
{
    ...
    if(ui->payTo->text().isEmpty()){
        ...
        if (!address.IsValid()) {
            QMessageBox::warning(this, tr("Spend Zerocoin"), tr("Invalid TUNEZ Address"), QMessageBox::Ok, QMessageBox::Ok);
            ui->payTo->setFocus();
            return;
        }
    }
    ...
    // Warn for additional fees if amount is not an integer and change as zTUNEZ is requested
    ...
    if(!fWholeNumber && fMintChange){
        ...
        strFeeWarning += QString::number(dzFee, 'f', 8) + " TUNEZ </b>will be added to the standard transaction fees!<br />";
    ...
    QString strAmount = "<b>" + QString::number(dAmount, 'f', 8) + " zTUNEZ</b>";
    ...
    // use mints from zTUNEZ selector if applicable
    ...
    if (walletModel && walletModel->getAddressTableModel()) {
        // If zTUNEZ was spent successfully update the addressbook with the label
    ...
    // Clear zTUNEZ selector in case it was used
    ...
    for (CZerocoinSpend spend : receipt.GetSpends()) {
        strStats += tr("zTUNEZ Spend #: ") + QString::number(nCount) + ", ";
    ...
    for (const CTxOut& txout: wtxNew.vout) {
        strStats += tr("value out: ") + FormatMoney(txout.nValue).c_str() + " TUNEZ, ";
        nValueOut += txout.nValue;

        strStats += tr("address: ");
        CTxDestination dest;
        if(txout.scriptPubKey.IsZerocoinMint())
            strStats += tr("zTUNEZ Mint");
                               ...
                               const CAmount& watchOnlyBalance, const CAmount& watchUnconfBalance, const CAmount& watchImmatureBalance)
{
    ...
    for (const auto& denom : libzerocoin::zerocoinDenomList) {
        ...
        strDenomStats = strUnconfirmed + QString::number(mapDenomBalances.at(denom)) + " x " +
                        ...
                        QString::number(nSumPerCoin) + " zTUNEZ </b>";
    ...
    ui->labelzAvailableAmount->setText(QString::number(zerocoinBalance/COIN) + QString(" zTUNEZ "));
    ui->labelzAvailableAmount_2->setText(QString::number(matureZerocoinBalance/COIN) + QString(" zTUNEZ "));
    ...
    ui->labelZsupplyAmount->setText(QString::number(chainActive.Tip()->GetZerocoinSupply()/COIN) + QString(" <b>zTUNEZ </b> "));
    for (auto denom : libzerocoin::zerocoinDenomList) {
        ...
        QString strSupply = QString::number(nSupply) + " x " + QString::number(denom) + " = <b>" +
                            QString::number(nSupply*denom) + " zTUNEZ </b> ";
        switch (denom) {
            case libzerocoin::CoinDenomination::ZQ_ONE:
