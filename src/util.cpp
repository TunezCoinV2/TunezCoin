...
bool LogAcceptCategory(const char* category)
{
    if (category != NULL) {
        ...
        if (ptrCategory.get() == NULL) {
            ...
            // "tunez" is a composite category enabling all TunezNG-related debug output
            if (ptrCategory->count(string("tunez"))) {
...
// Windows < Vista: C:\Documents and Settings\Username\Application Data\TunezNG
// Windows >= Vista: C:\Users\Username\AppData\Roaming\TunezNG
// Mac: ~/Library/Application Support/TunezNG
// Unix: ~/.tunez
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "TunezNG";
#else
    fs::path pathRet;
...
{
    boost::filesystem::path pathConfigFile(GetArg("-conf", "TunezNG.conf"));
    ...
    map<string, vector<string> >& mapMultiSettingsRet)
{
    ...
    if (!streamConfig.good()) {
        // Create empty tunez.conf if it does not exist
    ...
    for (boost::program_options::detail::config_file_iterator it(streamConfig, setOptions), end; it != end; ++it) {
        // Don't overwrite existing settings so command line settings override tunez.conf
...
{
    boost::filesystem::path pathPidFile(GetArg("-pid", "tunezd.pid"));
...
{
    namespace fs = boost::filesystem;
// Windows < Vista: C:\Documents and Settings\Username\Application Data\TunezCoinNG
// Windows >= Vista: C:\Users\Username\AppData\Roaming\TunezCoinNG
// Mac: ~/Library/Application Support/TunezCoinNG
// Unix: ~/.tunez
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "TunezCoinNG";
#else
    fs::path pathRet;
    char* pszHome = getenv("HOME");
    if (pszHome == NULL || strlen(pszHome) == 0)
...
{
    namespace fs = boost::filesystem;
    fs::path configFile = GetDefaultDataDir() / fs::path("TunezNG.conf");
    if (fs::exists(configFile))
        return; // new config file exists - skip renaming

    fs::path configFileLegacy = GetDefaultDataDir() / fs::path("TunezCoinNG.conf");
